...
public class Drivetrain extends SubsystemBase {
  ...
  public Drivetrain() {
    ...
    // starboardMotorSlave.follow(starboardMotor); // Set to follow the master
    // controller
  ...
  public void flyWithWiresB(double starboardMeters, double portMeters) {
    ...
    double starboardMiliseconds = ((starboardMeters / 100) * Constants.DrivetrainPID.rotationsPerMeter);
    double portMiliseconds = ((portMeters / 100) * Constants.DrivetrainPID.rotationsPerMeter);
  ...
  public boolean shiftGearboxesAutomatic(boolean targetGear) {
    if (targetGear == false && Math.abs(getOverallSpeed()) < Constants.Misc.Downshift_Max_Speed) {
      ...
      return true;
    } else if (targetGear == true && Math.abs(getOverallSpeed()) > Constants.Misc.Upshift_Min_Speed) {
